...
{
  Texto_Pais = Serial.readStringUntil('\n');
  deserializeJson(lista_pais, Texto_Pais);
  Serial.println(Texto_Pais);
  
  Nome_Pais = String(lista_pais["nome"]); 
  tela.fillRect(0,0,240,320,ILI9341_BLACK);
  x_t = 20;
  y_t = 200;
  tela.setCursor(x_t,y_t);
  tela.setTextColor(ILI9341_YELLOW);
  tela.setTextSize(4); 
  tela.print(Nome_Pais);


  
  for (unsigned int i =0; i< lista_pais["elementos"].size(); i++)
  {
    Serial.println("oi");
    if(lista_pais["elementos"][i]["tipo"]== "retângulo")
    {
      desenharRetangulo(lista_pais["elementos"][i]);
    }
    else if(lista_pais["elementos"][i]["tipo"]== "círculo")
    {
      desenharCirculo(lista_pais["elementos"][i]);
      
    }
    else
    {
      desenharPoligono(lista_pais["elementos"][i]);
