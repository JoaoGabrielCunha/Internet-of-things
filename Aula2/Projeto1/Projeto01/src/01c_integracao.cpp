...
void pagina1()
{
  ...
  arquivo.close();
  html.replace("{{horarioAcenderTudo}}",String(dados["horarioAcenderTudo"]));
  html.replace("{{horarioApagarTudo}}",String(dados["horarioApagarTudo"]));
  if (dados["alertaMovimento"]==true){
     html.replace("{{opcaoAlerta}}","checked");
  }
  else{
    html.replace("{{opcaoAlerta}}","");
  }


  if (dados["corRGB"]=="vermelho"){
     html.replace("{{opcaovermelho}}","checked");
  }
  else if(dados["corRGB"]=="amarelo"){
    html.replace("{{opcaoamarelo}}","checked");
  }
  else if(dados["corRGB"]=="rosa")
  {
    html.replace("opcaorosa","checked");

  }
  else if(dados["corRGB"]=="verde")
  {
    html.replace("opcaoverde","checked");
  }

 
  html.replace("{{opcaoAlerta}}",String(dados["alertaMovimento"]));
...
void tratarDados()
{
  String horarioAcenderTudo = servidor.arg("horarioAcenderTudo");
  String horarioApagarTudo  = servidor.arg("horarioApagarTudo");
  String corRGB             = servidor.arg("corRGB");
  String alertarMovimento   = servidor.arg("alertaMovimento");

  dados["horarioAcenderTudo"] = horarioAcenderTudo;
  dados["horarioApagarTudo"]  = horarioApagarTudo;
  dados["corRGB"]  = corRGB;
  dados["alertarMovimento"] = alertarMovimento;
  
  
  File arquivo = LittleFS.open("/ajustes.json","w");
...
void setup()
{
  ...
  servidor.on("/inicio", HTTP_POST, tratarDados);
  servidor.on(UriBraces("/led/{}/{}"),HTTP_GET,pagina2);
  servidor.begin();
